...
exports.loadApiRoutes = (app) => {
    ...
    app.use('/api/v1/auth',Headers,Useragent.ApiHeaders,routerV1);

    // Room API routes V1:
    const {RoomRouterV1} = require(path.join(appDir,'app','Modules/Room/routes/api.js'));
    app.use('/api/v1/rooms',Headers,Useragent.ApiHeaders,RoomRouterV1);

    // Chat API routes V1:
    const {ChatRouterV1} = require(path.join(appDir,'app','Modules/Room/routes/api.js'));
    app.use('/api/v1/chats',Headers,Useragent.ApiHeaders,ValidRoom,ChatRouterV1);

    // Setting API routes V1:
    const {settingRouterV1} = require(path.join(appDir,'app','Modules/Settings/routes/api.js'));
    app.use('/api/v1/settings',Headers,Useragent.ApiHeaders,settingRouterV1);
}